<?php namespace Tukecx\Base\ModulesManagement\Console\Generators;
...
class MakeModule extends Command
{
     ...
     * @var string
     */
    protected $description = 'Tukecx modules generator.';
    ...
    private function step1()
    {
        ...
        $this->container['name'] = $this->ask('Name of module:', 'Tukecx ' . $this->container['alias']);
        ...
        $this->container['namespace'] = $this->ask('Namespace of module:', 'Tukecx\\' . $this->acceptedTypes[$this->moduleType] . '\\' . studly_case($this->container['alias']));
    ...
    private function makeModuleFolder()
    {
        switch ($this->moduleType) {
            case 'base':
                if (!$this->files->isDirectory(tukecx_base_path())) {
                    $this->files->makeDirectory(tukecx_base_path());
                }
                return 'tukecx_base_path';
                break;
            case 'plugin':
            default:
                if (!$this->files->isDirectory(tukecx_plugins_path())) {
                    $this->files->makeDirectory(tukecx_plugins_path());
                }
                return 'tukecx_plugins_path';
                break;
        }
    }
